...
- Flavors: `foss` (default: no Play Services, no Firebase — plan decision I8) and `gms` (FCM wake-ups only, in `app/src/gms`); app host tests run on `foss`. The relay host, extra certificate pins and Firebase options are Gradle properties or environment variables (README "Build settings"); never commit `google-services.json`, API keys, keystores or service-account files.
