...
Gradle reads `../shared/design-tokens/tokens.json` (task `:core:design:generateHandLiveTheme`); unit tests read `../shared/test-vectors` and `../shared/schemas` through the system property `hl.shared.dir`. Outside this layout nothing builds. From the hub, `tools/workspace.sh clone <group-url>` checks the parts out and `tools/workspace.sh status` shows all five repositories.
...
- Build and test: `./gradlew check` (JDK 21; `local.properties` → `sdk.dir` with `platforms;android-36`). Instrumented tests run on a real device when the task card says so.
...
- CI (`.github/workflows/ci-android.yml`) reproduces the layout: this repo into `android/`, handlive-shared into `shared/`. It does not run when only shared changes — start it by hand (workflow_dispatch).
